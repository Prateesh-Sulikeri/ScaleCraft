...
depict this in diagrams throughout the curriculum, but no registry component
accepts one on canvas yet (checked directly against `load-balancer` and
`app-server`'s own `relations` fields) - health checking is real and this
chapter relies on it, but it isn't a wire you'll find to draw.
