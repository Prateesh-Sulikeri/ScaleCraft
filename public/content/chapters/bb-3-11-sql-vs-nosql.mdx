...
> [!NOTE]
> Think first: the catalog above is one table, fighting its own shape every
> sprint. Would you reach for a different kind of database here, or fix it
> some other way? Commit to an answer before reading on.
...
Note: shape and access pattern get asked before scale - a workload that
