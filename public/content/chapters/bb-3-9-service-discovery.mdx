...
its own, hand-picking an instance count stops working, because nothing is
tracking which instances currently exist. That's this chapter's problem.
...
> [!NOTE]
> Think first: if the load balancer already knows which backends are alive,
> what's actually new here? Consider what happens once there's more than one
> internal service, not just one client-facing tier. Commit to an answer
> before reading on.
...
Note: the registry, not the caller, tracks who's currently healthy -
...
and it still isn't something you can wire on canvas - shown here in diagrams
and prose only, the same gap 3.4 and 3.8 already named.
