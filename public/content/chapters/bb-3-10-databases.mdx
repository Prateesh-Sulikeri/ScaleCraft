...
> [!NOTE]
> Think first: you scaled the app tier by adding identical copies behind a
> load balancer. Why doesn't the same move work here - just point half the app
> servers at a second, independent database and call it solved? Commit to an
...
Note: 3.8 let you add app-server copies for free - the load balancer
...
Note: with no index, a lookup costs one check per row - a scan. An index
