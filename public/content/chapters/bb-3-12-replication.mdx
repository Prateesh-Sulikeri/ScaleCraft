...
> [!NOTE]
> Think first: if you added a second copy of the primary database, just to
> absorb read traffic, what could go wrong that adding a second app-server
> copy (3.8) never had to worry about? Commit to an answer before reading on.
...
Note: the replication edge only flows primary to replica - a write never
travels the other way. Reads flow back out to the Application Server from the
Replica; writes have nowhere to land but the primary.
...
Note: the write is acknowledged before replication catches the replica up
