...
| 14 Connections + Preview of next | "Next" | Backward: 1.6 (named and deferred this component) and 1.7 (its methodology now applies to a real multi-instance system), both named explicitly - satisfies §19's >=2. Forward: 3.8, this chapter's one tease. |
...
- **Word count.** 1,242 words (lesson body) - compare against 1.6's 950 for
  a 30-minute chapter; this one is 35 minutes and covers two algorithms
  plus a failure-mode shift, which is the justification for the difference,
  not padding. Worth a second look during the density pass.
