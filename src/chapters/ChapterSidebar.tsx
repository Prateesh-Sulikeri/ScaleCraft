...
/**
 * The in-workspace sidebar — a "Back to lesson" / "Next chapter" row above
 * the always-rendered QuestionPane. The Design Editor's own route to a
 * chapter is now reached exclusively through the Chapter Reader
 * (`/<mode>/<slug>/lesson`'s Design Editor CTA), so "Back to lesson" is the
 * one way back out: no standalone curriculum browser or "View full Learning
 * Path" link here (that's the Reader's ReaderSidebar's job) — going back
 * always means going back to the lesson this canvas belongs to. "Next
 * chapter" (NextChapterLink) is the one deliberate exception — always
 * available, not gated on passing this chapter, so a learner can move on
 * without being forced back through the lesson first.
 */
export function ChapterSidebar({
  ...
  return (
    <div data-tour="question-pane" className="flex min-h-0 flex-1 flex-col">
      <div className="flex shrink-0 items-center justify-between border-t border-b border-border px-3 py-2">
        <HeldTransitionLink
        ...
        <NextChapterLink courseId={courseId} chapterSlug={chapterSlug} className="text-xs text-foreground/70" />
      </div>
      <QuestionPane
        chapter={chapter}
        entry={entry}
